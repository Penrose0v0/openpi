#!/usr/bin/env bash
# One-shot Baseline-without-wrist (object set) experiment:
#   1. starts the pi05 server for the no-wrist baseline (config pi05_libero_baseline_no_wrist_lora,
#      trained on libero_view45 with the wrist view dropped -> served with use_wrist=False,
...
#   examples/libero/run_baseline_no_wrist_object.sh
#   CKPT_DIR=/path/to/ckpt AZIMUTHS="0 45 90" SERVER_GPU=1 SIM_GPU=0 examples/libero/run_baseline_no_wrist_object.sh
set -euo pipefail
...
CKPT_DIR="${CKPT_DIR:-/root/workspace/checkpoints/pi05_libero_baseline_lora/libero_baseline_without_wrist/29999}"
POLICY_CONFIG="${POLICY_CONFIG:-pi05_libero_baseline_no_wrist_lora}"
TASK_SUITE="${TASK_SUITE:-libero_object}"
AZIMUTHS="${AZIMUTHS:-0 45 90}"
SERVER_GPU="${SERVER_GPU:-7}"          # GPU for the policy server
SIM_GPU="${SIM_GPU:-7}"                # GPU for MuJoCo/EGL rendering
...
SERVER_LOG="$REPO/baseline_no_wrist_server.log"
...
echo "[orchestrator] starting Baseline-no-wrist server"
...
# ---- run the sweep ----------------------------------------------------------
echo "[orchestrator] eval: model=baseline_nowrist suite=$TASK_SUITE azimuths='$AZIMUTHS' (sim on GPU $SIM_GPU)"
MODEL_TAG=baseline_nowrist TASK_SUITE="$TASK_SUITE" AZIMUTHS="$AZIMUTHS" PORT="$PORT" \
